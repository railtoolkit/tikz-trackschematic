...
%
%%%%%%%%%%%%%%%
% symbol trigger point
%%%%%%%%%%%%%%%
% command
\newcommand\triggerpoint{}% just for safety
\def\triggerpoint[#1]#2(#3){% \triggerpoint[options] at (coord);
  \pic[#1] at (#3) {trigger_point={#2}};% symbol
}%
% symbol definition
\tikzset{%
  pics/trigger_point/.default=,%
  pics/trigger_point/.style args={#1}{code={%
    %% face setup
    \ifdefstring{\face}{forward}{% face
      \pgfmathsetmacro{\facefactor}{1}%
    }{%
    ...
    %% rings
    \path[draw=foreground, line width=1pt]% circle
      ($\facefactor*(0.4,0) + \facefactor*\trafficfactor*(0,-0.4)$) circle (0.1);
    \path[fill=background]% line
      ($\facefactor*(0.45,0) + \facefactor*\trafficfactor*(0,-0.45)$) rectangle ++($\facefactor*(0.1,0) + \facefactor*\trafficfactor*(0,0.1)$);
    \path[draw=foreground, line width=1pt]% line
      ($\facefactor*(0.425,0) + \facefactor*\trafficfactor*(0,-0.4)$) -- ++($\facefactor*(0.15,0)$);
  }},% END pics/trigger_point/.style args={#1}
  % symbology entry
  symbology_trigger_point/.pic = {%
    \maintrack (0,0) -- (6,0);%
    \triggerpoint[forward] at (3,0);%
  },%
}%
%
%%%%%%%%%%%%%%%
